...
MODEL_SAVE_LOCATION = "./generated/DeepMinds-Robosoccer2v2-v1"
...
class RoboSoccerGym(gym.Env):
    def __init__(self, env_name, n_players, config=None, modify_to_distances=True, modify_to_field_image=False):
        ...
        self.observation_size = (3 * (2 * self.number_players)) + 3 * 2
...
if __name__ == "__main__":
    ...
    check_env(env=RoboSoccerGym(
        env_name="Robosoccer2v2-v1", n_players=2), warn=True)
    ...
    env = RoboSoccerGym(env_name="Robosoccer2v2-v1", n_players=n_players)
    model = A2C(
        "MlpPolicy",
        env,
